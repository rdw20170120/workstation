...
def check_host(host, port=8091, timeout=5):
    ...
    if result != 0:
        raise HostException('{0}:{1} : Host not reachable'
                            .format(host, port))
    sock.close()
...
def connect_ssh(host, username, SSHKey_path):
    """Connecting to a remote host."""
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    # Connecting to host
    ssh.connect(host, username=username, key_filename=SSHKey_path)
    return ssh
...
def generate_output(client_host, user, SSHKey, filename, content):
    """Create a output file in the node with the govent content."""
    directory = os.path.dirname(filename)
    ssh = connect_ssh(client_host, user, SSHKey)
    commands = [
...
def raise_exception(stdout, stderr):
    """Raising excepiton to the failed commands"""
    exception = ''
    stdout = stdout.read()
    if stdout:
