...
def ck001(ssh, server_host):
    score = 0
    msg = '{0}: NTP is not running'.format(server_host)
    ntp_status = common.get_cmd_status(
        ssh, 'sudo systemctl status ntpd | grep Active', 'active (running)'
    )
    if ntp_status:
        score = 1
        msg = '{0}: NTP is running'.format(server_host)
    return score, msg


def ck002(ssh, server_host):
    score = 0
    msg = '{0}: CB server is not disabled'.format(server_host)
    ...
    if exit_status == 3:
        ...
        if 'is not running' in output:
            score = 1
            msg = '{0}: CB server is disabled'.format(server_host)
    return score, msg


def ck003(ssh, server_host):
    score = 0
    msg = '{0}: ntpstat is not synchronized with server'.format(server_host)
    ntpstat_status = common.get_cmd_status(
        ssh, 'ntpstat', 'synchronised to NTP server'
    )
    if ntpstat_status:
        score = 1
        msg = '{0}: ntpstat is synchronized with server'.format(server_host)
    return score, msg


def ck004(ssh, server_host):
    score = 0
    msg = '{0}: Swapiness is not configured'.format(server_host)
    ...
    try:
        ...
        msg = '{0}: {1}'.format(server_host, e)
    else:
        if swap_int == 0:
            score = 1
            msg = '{0}: Swapiness is configured'.format(server_host)
    return score, msg


def ck005(ssh, server_host):
    score = 0
    msg = '{0}: THP is not disabled'.format(server_host)
    ...
    if thp_enable and thp_defrag:
        score = 1
        msg = '{0}: THP is disabled'.format(server_host)
    return score, msg


def ck006(ssh, server_host):
    score = 0
    msg = "{0}: Couchbase is not running".format(server_host)
    ...
    if cb_status:
        score = 1
        msg = "{0}: Couchbase is running".format(server_host)
    return score, msg


def ck007(server_host):
    score = 0
    msg = "{0}: Not reachable at 8091".format(server_host)
    if common.grading_host_check(server_host):
        score = 1
        msg = "{0}: Reachable at 8091".format(server_host)
    return score, msg
...
def ck009(server_host, node_services, services):
    score = 0
    msg = "{0}: Required services are not configured".format(server_host)
    if all(serv.strip() in node_services for serv in services):
        score = 1
        msg = "{0}: Required services are configured".format(server_host)
    return score, msg


def ck010(server_host, node_services):
    score = 0
    msg = "{0}: FTS service is configured".format(server_host)
    if 'fts' not in node_services:
        score = 1
        msg = "{0}: FTS service is not configured".format(server_host)
    return score, msg


def ck011(server_host, node_data, data_path, index_path):
    hdd_data = node_data.get('storage').get('hdd')
    score = 0
    msg = "{0}: Data and index directories not configured".format(server_host)
    if(hdd_data and hdd_data[0].get('path') == data_path and
       hdd_data[0].get('index_path') == index_path):
        score = 1
        msg = (
...
def ck012(server_host, response):
    score = 0
    msg = "{0}: Unable to read the cluster information".format(server_host)
    if response:
        score = 1
        msg = "{0}: Superuser credentials configured".format(server_host)
    return score, msg


def ck013(node, server_host):
    msg = "{0}: Added to the cluster".format(server_host)
    return 1, msg
...
def ck015(hostname, response, bucketname):
    score = 0
    msg = '{0}: Unable to read buckets information'.format(hostname)
    if response and response.status_code == 200:
        msg = '{0}: {1} - Bucket not exists'.format(hostname, bucketname)
            ...
            if(bucketname == bucket.get('name') and
               bucket.get('bucketType') == 'membase'):
                ...
                msg = (
                    ...
                    .format(hostname, bucketname)
                )
                break
    return score, msg


def ck016(hostname, response, bucketname, doc_count=280322):
    for bucket in response.json():
...
def ck018(response, cluster_host, bucket_name, index_name):
    """Checking the index created with correct definition on the
       specified bucket."""
    score = 0
    msg = "{0}: Failed to get index status".format(cluster_host)
    if response:
            ...
            if(index.get('bucket') == bucket_name and
               ...
               index.get('definition').lower().startswith('create index')):
                ...
                msg = (
                    ...
                    .format(cluster_host, bucket_name)
                )
                break
        else:
            msg = (
    ...
    if results:
        ...
        if sorted(doc_types) == sorted(file_doc_types):
            ...
            msg = (
                ...
                ' no errors'.format(client_host, file)
            )
    return score, msg


def ck020(results, client_host, file):
    ...
    if results:
        ...
        if sorted(list(doc_types.keys())) == sorted(file_doc_types):
            ...
            msg = (
                ...
                'and no errors'.format(client_host, file)
            )
    return score, msg


def ck021(results, client_host, file):
...
def ck022(results, cluster_host, client_host, bucket_name, file, auth, key):
    score = 0
    msg = (
        '{0}: {1} file contains errors/incorrect result count'
        .format(client_host, file)
    ...
    if results and len(results) == 1:
        ...
        try:
            response = common.execute_query(
                cluster_host, step06_query, auth
            )
            json_response = response.json()
        except Exception as e:
            msg = '{0}: {1}'.format(client_host, e)
        else:
            msg = '{0}: Output contains wrong result'.format(client_host)
...
def ck023(results, client_host, bucket_name, file, key):
    score = 0
    msg = (
...
def ck024(size_status, app_srvr_hostname, stdo):
    score = 0
    msg = "{0}: Unable to read fileSize".format(app_srvr_hostname)
    if size_status == 0:
        try:
        ...
        if file_size >= 323 and file_size <= 333:
            ...
            msg = (
                ...
                .format(app_srvr_hostname, file_size)
            )
        else:
            msg = (
                "{0}: Backup file size must be in between 323M"
                " and 333M".format(app_srvr_hostname)
            )
    return score, msg


def ck025(response, source_host, source_bucket, dest_bucket, uuid):
    score = 0
    msg = "{0}: Unable to read the tasks information".format(source_host)
    if response.status_code == 200:
        for task in response.json():
            if task.get('type') == 'xdcr' and \
               task.get('status') == 'running' and \
               task.get('source') == source_bucket and \
               task.get('target').endswith(dest_bucket) and \
               uuid in task.get('target'):
                score = 1
                msg = "{0}: XDCR replication created".format(source_host)
                break
        else:
            msg = (
...
def ck028(nodes, fqdn_list, cluster_host):
    score = 0
    msg = "{0}: Nodes not removed from cluster".format(cluster_host)
    nodes_deleted = True
    for node in nodes:
    ...
    if nodes_deleted and fqdn_list:
        score = 1
        msg = "{0}: Nodes removed from cluster".format(cluster_host)
    return score, msg


def ck029(ssh, server_host):
    score = 0
    msg = "{0}: CB server is not uninstalled".format(server_host)
    status, stdout, stderr = common.execute_command(
        ssh, 'sudo /etc/init.d/couchbase-server status'
    )
    if(status == 1):
        score = 1
        msg = "{0}: CB server is uninstalled".format(server_host)
    return score, msg


def ck030(response, cluster_host):
    score = 0
    msg = "{0}: {1}".format(cluster_host, response.reason)
    if response.status_code == 200:
        ...
        for node in nodes:
            ...
            score = 1
            msg = "{0}: Cluster is rebalanced".format(cluster_host)
    return score, msg


def ck031(response, auth, dest_host, src_host, port, src_bucket, dest_bucket):
    score = 0
            ...
            if res.status_code == 200:
                for task in res.json():
                    if task.get('type') == 'xdcr' and \
                       ...
                       dest_bucket in task.get('target') and \
