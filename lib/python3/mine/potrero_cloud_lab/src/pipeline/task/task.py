...
class Task:
    ...
    def _delete_output_directory_upon_exception(self, directory_path):
        log.warn(
    ...
    def _delete_output_file_upon_exception(self, file_path):
        if file_path.exists():
            log.warn(
    ...
    def _get_s3(self):
        return S3(c.profile_name, c.region_name)

    def execute(self):
        log.info("Executing: %s", self)
        self._skip_for_lack_of_disk_space(c.reserved_space_in_bytes)
