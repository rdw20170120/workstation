...
def _comments():
    return [
        ...
        comment('this, then please experiment by uncommenting the ',
            cc(set_('-x')), ' command above and',
        ),
        comment("activating a new shell.  Having learned that lesson, let's never use ",
            cc(set_('-x')),
        ),
        comment('in a BASH script intended to be ', cc(command('source')), 'd.  Similarly, the ',
            cc(set_('-e')), ' BASH',
        ),
        comment('option is problematic because its haphazard behavior does not deliver on its'),
