...
def test_encounter_1_Hz_file():
    f = (
        ...
        monitor /
        ('no_phi-encounter_099-1_Hz' + c.stage_7_file_suffix)
        )
...
def test_encounter_100_Hz_file():
    f = (
        c.intermediate_directory /
        monitor /
        ('no_phi-encounter_099-100_Hz' + c.stage_7_file_suffix)
        )
...
def test_encounter_sparse_file():
    f = (
        c.intermediate_directory /
        monitor /
        ('no_phi-encounter_099-sparse' + c.stage_7_file_suffix)
        )
...
def test_file_is_monitor_archive():
    ...
    assert not m.file_is_monitor_archive(f)
    f = Filename('SOMETHING' + c.stage_7_file_suffix)
    assert not m.file_is_monitor_archive(f)
...
def test_map_monitor_to_anonymized_encounter_file():
    n = 'encounter' + c.stage_6_file_suffix
    f = c.intermediate_directory / monitor / n
    assert m.map_monitor_to_anonymized_encounter_file(monitor) == f
